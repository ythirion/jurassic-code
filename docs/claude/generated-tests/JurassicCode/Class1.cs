...
public interface IParkService
{
    ...
    void RemoveDinosaurFromZone(string zoneName, string dinosaurName);
}
...
public partial class ParkService : IParkService
{
    ...
    public void MoveDinosaur(string fromZoneName, string toZoneName, string dinosaurName)
    {
        ...
        if (fromZone != null && toZone != null)
        {
            ...
            {
                throw new Exception($"Dinosaur with name {dinosaurName} does not exist in zone {fromZoneName}.");
            }
        }
        else
        {
    ...
    public void RemoveDinosaurFromZone(string zoneName, string dinosaurName)
    {
        bool zoneFound = false;
        ...
        foreach (var zone in DataAccessLayer._db.Zones)
        {
            ...
            {
                zoneFound = true;
                for (int i = 0; i < zone.Value.DinosaurCodes.Count; i++)
                {
                    if (zone.Value.DinosaurCodes[i] == dinosaurName)
                    {
                        zone.Value.DinosaurCodes.RemoveAt(i);
                        dinosaurFound = true;
                        break;
                    }
                }
        ...
        if (!dinosaurFound)
        {
            throw new Exception("Dinosaur not found in zone.");
        }
    }
}
